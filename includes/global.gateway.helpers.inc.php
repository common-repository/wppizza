...
*	@return str
******************************************************************/
function wppizza_webhook_url($gateway_slug){
	/* sanitize, just for good measure */
	$webhook = strtolower(wppizza_alpha_only(WPPIZZA_LISTENER_PARAMETER));
	$webhook_url = site_url().'/?'.$webhook.'='.wppizza_alpha_only($gateway_slug);
return $webhook_url;
}
/******************************************************************
*
...
/******************************************************************
*
*	[sum the surcharges - added in v3.16 - if any ]
*	@since 3.16
*	@return float/int
******************************************************************/
function wppizza_surcharges_total($order_details){
	$surcharges_total = 0;
	if(!empty($order_details['summary']['surcharges'])){
		$values = wppizza_array_column($order_details['summary']['surcharges'], 'value');
		if(!empty($values) && is_array($values)){
			$surcharges_total = wppizza_round(array_sum($values));	
		}
	}
	return $surcharges_total;
}
?>