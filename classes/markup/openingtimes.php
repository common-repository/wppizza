...
class WPPIZZA_MARKUP_OPENINGTIMES{
	...
	***************************************/
	...
	/***************************************
		[markup]
	***************************************/
	function get_markup($atts, $widget_wrapper = true){	
		...
		if(!empty($atts['type'])  && $atts['type'] != 'openingtimes'){
		...
		*/
		...
		/*
			omit wrapper (for example if using ajax to re-fill widget content)
		*/
		if(empty($widget_wrapper)){
			unset($markup['div_']);
			unset($markup['_div']);
		}
	...
	***************************/
	...
	function days_concat($days){
	    ...
	    $output = preg_split( '/##+/', $output, 0, PREG_SPLIT_NO_EMPTY );
